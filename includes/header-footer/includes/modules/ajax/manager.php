...
if (!class_exists('Thepack_Kit_Ajax_Manager')) {
     ...
     * @since 1.0.0
     */
    class Thepack_Kit_Ajax_Manager
    {
	    ...
	    public function register_ajax_action( $tag, $callback ) {
		    if ( ! did_action( 'thepack-kit/ajax/register_actions' ) ) {
			    _doing_it_wrong( __METHOD__, esc_html( sprintf( 'Use `%s` hook to register ajax action.', 'thepack-kit/ajax/register_actions' ) ), '1.1.2' );
		    }
	     ...
	     *
	     * @return Thepack_Kit_Ajax_Manager An instance of ajax manager.
	     */
