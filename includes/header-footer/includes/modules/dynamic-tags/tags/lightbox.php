...
class Lightbox extends Tag {
	...
	private function get_image_settings( $settings ) {
		...
		if ( $image_id ) {
			$lightbox_image_attributes = thepack_kit()->elementor()->images_manager->get_lightbox_image_attributes( $image_id );
	...
	public function render() {
		...
		// PHPCS - the method thepack_kit()->elementor()->frontend->create_action_hash is safe.
		echo thepack_kit()->elementor()->frontend->create_action_hash( 'lightbox', $value ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	}
}
