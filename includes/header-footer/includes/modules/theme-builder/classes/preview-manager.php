...
class Preview_Manager {
	...
	public function __construct() {
		...
		add_filter( 'thepack-kit/dynamic_tags/post_terms/taxonomy_args', [ $this, 'filter_post_terms_taxonomy_arg' ] );
	...
	public function filter_query_control_args( $query_vars ) {
		$document = thepack_kit()->elementor()->documents->get_doc_or_auto_save( get_the_ID() );
	...
	public function switch_to_preview_query() {
		$current_post_id = get_the_ID();
		$document = thepack_kit()->elementor()->documents->get_doc_or_auto_save( $current_post_id );
		...
		thepack_kit()->elementor()->db->switch_to_query( $new_query_vars, true );
	...
	public function restore_current_query() {
		thepack_kit()->elementor()->db->restore_current_query();
	}
}
