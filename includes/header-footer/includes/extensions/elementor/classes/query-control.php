...
namespace ThePackKitExtensions\Elementor\Classes;
...
class Query_Control {
    ...
    public function get_name() {
        return 'thepackkit-query-control';
    }
    ...
    public function ajax_posts_filter_autocomplete( array $data ) {
        ...
        switch ( $data['filter_type'] ) {
            ...
            case 'author':
                ...
                break;
            default:
                $results = apply_filters( 'thepack_kit/query_control/get_autocomplete/' . $data['filter_type'], [], $data );
        }
    ...
    private function ajax_posts_filter_autocomplete_new( $data ){
        ...
        switch ( $query_data['object'] ) {
            ...
            case self::QUERY_OBJECT_LIBRARY_TEMPLATE:
                ...
                foreach ( $query->posts as $post ) {
                    $document = thepack_kit()->elementor()->documents->get( $post->ID );
                    if ( $document ) {
    ...
    public function ajax_posts_control_value_titles( $request ) {
        ...
        switch ( $request['filter_type'] ) {
            ...
            case 'author':
                ...
                break;
            default:
                $results = apply_filters( 'thepack_kit/query_control/get_value_titles/' . $request['filter_type'], [], $request );
        }
